...
const OrdenCompraDetalleEditComponent = ({ciudad, onCancelar, onGuardadoExitoso }) => { //esto es lo que manda para saber cuando cerrar el crear
  const [ordenesCompras, setOrdenesComprasDetalle] = useState([]);

  const [openSnackbar, setOpenSnackbar] = useState(false); 
  const apiUrl = process.env.REACT_APP_API_URL;
  const apiKey = process.env.REACT_APP_API_KEY;
  const listarOrdenCompraDetalles = () => {
          axios.get(`${apiUrl}/api/OrdenCompraDetalles/Listar?pvin_EsAduana=true`, {
            headers: {
              'XApiKey': apiKey
            }
          })
  ...
  const formik = useFormik({
        ...
        axios.post(`${apiUrl}/api/OrdenCompraDetalles/Editar`, datosParaEnviar, {
        headers: { 
            'XApiKey': apiKey }
        })
        .then(() => {
            ...
            <Grid container spacing={3} mb={3}>  {/* Esto es como el div con class row */}
                ...
                <Grid item lg={6} md={12} sm={12}>             
                    ...
                    <CustomTextField
                        ...
                        helperText={formik.touched.proc_Id && formik.errors.proc_Id}
                    >  
                    {ordenesComprasDetalles.map((ordenCompraDetalle) => (
                ...
                <Grid item lg={6} md={12} sm={12}>
                    ...
                    >
                    {/* {ordenCompraDetalles.map((fecha) => (
                        <MenuItem key={fecha.code_FechaProcActual} value={fecha.code_FechaProcActual}>
                            {fecha.code_FechaProcActual}
                        </MenuItem>
                    ))} */}
                    </CustomTextField>
                </Grid>
