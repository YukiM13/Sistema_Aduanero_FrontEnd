...
const TratadosMasUsadosChart = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`${apiUrl}/api/AduanasGraficas/TratadosMasUsados`, {
      headers: {
        'XApiKey': apiKey
      }
    })
  ...
  const totalProduccion = modulosData.reduce((total, modulo) => 
    total + modulo.cantidad, 0);

  return (
    <DashboardCard 
      title="Tratados Mas Utilizados" 
      subtitle=""
      action={
        <IconWorld
        ...
        <Box p={0}>
          <Stack spacing={3}>
            {modulosData.map((modulo, i) => {
              ...
              const porcentaje = parseFloat(modulo.porcentaje).toFixed(2);
                  ...
                  >
                    <Box>
                      <Typography variant="subtitle1" fontWeight="600">
                        {modulo.trli_NombreTratado}
                      </Typography>
                      <Typography variant="body2" color="textSecondary">
                        Acuerdos: {new Intl.NumberFormat('es-HN').format(modulo.cantidad)}
                      </Typography>
                    </Box>
                    <Chip
                      sx={{
          ...
          <Box mt={3} pt={3} sx={{ borderTop: `1px solid ${theme.palette.divider}` }}>
            <Typography variant="subtitle1" fontWeight="600">
              Total de Acuerdos: {new Intl.NumberFormat('es-HN').format(totalProduccion)}
            </Typography>
          </Box>
        </Box>
      )}
...
export default TratadosMasUsadosChart;
