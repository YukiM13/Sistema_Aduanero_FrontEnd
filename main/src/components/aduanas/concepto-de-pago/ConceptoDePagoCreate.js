...
import ConceptoDePago from 'src/models/conceptopago';
...
const validationSchema = yup.object({
  copa_Descripcion: yup.string().required('El código es requerido'),
});


   
const ConceptosDePagoCreateComponent = ({ onCancelar, onGuardadoExitoso }) => { //esto es lo que manda para saber cuando cerrar el crear
...
const [openSnackbar, setOpenSnackbar] = useState(false); 
  ...
  const formik = useFormik({
        ...
        initialValues: ConceptoDePago,
        validationSchema,
        onSubmit: (values) => {
          values.copa_FechaCreacion = new Date().toISOString();
          values.usua_UsuarioCreacion = 1;
          console.log("Valores antes de enviar:", values);
          axios.post(`${apiUrl}/api/ConceptoPago/Insertar`, values, {
          ...
          .catch(error => {
            console.error('Error al insertar el concepto de pago:', error);
          });
...
export default ConceptosDePagoCreateComponent;