...
const MarcaCreateComponent = ({ onCancelar, onGuardadoExitoso }) => { //esto es lo que manda para saber cuando cerrar el crear
...
const [openSnackbar, setOpenSnackbar] = useState(false); 
  ...
  const formik = useFormik({
        ...
        initialValues: MarcaModel,
        validationSchema,

        onSubmit: (values) => {
          values.marc_FechaCreacion = new Date();
          values.marc_FechaModificacion = new Date();
          values.usua_UsuarioCreacion = 1;
          

          axios.post(`${apiUrl}/api/Marcas/Insertar`, values, {
...
export default MarcaCreateComponent;
