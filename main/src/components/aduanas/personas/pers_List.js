...
const PersonasComponent = () => {
  ...
  const eliminar = (persona) => {
    axios.post(`${apiUrl}/api/Personas/Eliminar`, persona, { headers: { 'XApiKey': apiKey } })
      .then(() => {
        cargarPersonas();
  ...
  return (
    <div>
      ...
      <ParentCard>
        ...
        {/* Add components for 'crear', 'editar', and 'detalle' modes */}
      </ParentCard>
      <Snackbar
        open={openSnackbar}
      ...
      <Dialog open={confirmarEliminacion} onClose={() => setConfirmarEliminacion(false)}>
        ...
        <DialogActions>
          <Button onClick={() => setConfirmarEliminacion(false)} variant="outlined" color="primary">
            Cancelar
          </Button>
          <Button
            onClick={() => {
